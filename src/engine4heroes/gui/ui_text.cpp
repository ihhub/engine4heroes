...
namespace
{
    ...
    const engine4heroes::Sprite errorImage;

    const engine4heroes::Sprite & getChar( const uint8_t character, const engine4heroes::FontType & fontType )
    {
        if ( character < 0x21 ) {
            return errorImage;
        }
        ...
        case engine4heroes::FontSize::SMALL:
            ...
            case engine4heroes::FontColor::WHITE:
                return Assets::getImage( ImageId::FONT_PROSE_ANTIQUE_14, character );
            case engine4heroes::FontColor::GRAY:
                return Assets::getImage( ImageId::FONT_PROSE_ANTIQUE_16, character );
            case engine4heroes::FontColor::YELLOW:
                return Assets::getImage( ImageId::FONT_PROSE_ANTIQUE_18, character );
            default:
        ...
        case engine4heroes::FontSize::NORMAL:
            switch ( fontType.color ) {
            case engine4heroes::FontColor::WHITE:
                return Assets::getImage( ImageId::FONT_PROSE_ANTIQUE_20, character );
            case engine4heroes::FontColor::GRAY:
                return Assets::getImage( ImageId::FONT_PROSE_ANTIQUE_20, character );
            case engine4heroes::FontColor::YELLOW:
                return Assets::getImage( ImageId::FONT_PROSE_ANTIQUE_20, character );
        ...
        case engine4heroes::FontSize::LARGE:
            switch ( fontType.color ) {
            case engine4heroes::FontColor::WHITE:
                return Assets::getImage( ImageId::FONT_PROSE_ANTIQUE_26, character );
            case engine4heroes::FontColor::GOLDEN_GRADIENT:
                return Assets::getImage( ImageId::FONT_PROSE_ANTIQUE_26, character );
            case engine4heroes::FontColor::SILVER_GRADIENT:
                return Assets::getImage( ImageId::FONT_PROSE_ANTIQUE_26, character );
        ...
        case engine4heroes::FontSize::BUTTON_RELEASED:
            switch ( fontType.color ) {
            case engine4heroes::FontColor::WHITE:
                return Assets::getImage( ImageId::FONT_PROSE_ANTIQUE_10, character );
            case engine4heroes::FontColor::GRAY:
                return Assets::getImage( ImageId::FONT_PROSE_ANTIQUE_10, character );
            default:
                // Did you add a new font color? Add the corresponding logic for it!
    ...
    uint32_t getCharacterLimit( const engine4heroes::FontSize fontSize )
    {
        switch ( fontSize ) {
        case engine4heroes::FontSize::SMALL:
            return 255;
    ...
    int32_t getLineWidth( const uint8_t * data, const int32_t size, const engine4heroes::FontCharHandler & charHandler, const bool keepTrailingSpaces )
    {
    ...
    engine4heroes::Rect getTextLineArea( const uint8_t * data, const int32_t size, const engine4heroes::FontCharHandler & charHandler )
    {
        assert( data != nullptr && size > 0 );

        const uint8_t * dataEnd = data + size;

        const engine4heroes::Sprite & firstCharSprite = charHandler.getSprite( *data );
        engine4heroes::Rect area( firstCharSprite.x(), firstCharSprite.y(), firstCharSprite.width(), firstCharSprite.height() );
        ++data;

        for ( ; data != dataEnd; ++data ) {
            const engine4heroes::Sprite & sprite = charHandler.getSprite( *data );
    ...
    int32_t getMaxCharacterCount( const uint8_t * data, const int32_t size, const engine4heroes::FontCharHandler & charHandler, const int32_t maxWidth )
    {
                              ...
                              const engine4heroes::Rect & imageRoi, const engine4heroes::FontCharHandler & charHandler )
    {
        ...
        for ( ; data != dataEnd; ++data ) {
            ...
            const engine4heroes::Sprite & charSprite = charHandler.getSprite( *data );
            assert( !charSprite.empty() );

            const engine4heroes::Rect charRoi{ offsetX + charSprite.x(), y + charSprite.y(), charSprite.width(), charSprite.height() };
    ...
    int32_t getMaxWordWidth( const uint8_t * data, const int32_t size, const engine4heroes::FontType fontType )
    {
        assert( data != nullptr && size > 0 );

        const engine4heroes::FontCharHandler charHandler( fontType );
    ...
    std::unique_ptr<engine4heroes::LanguageSwitcher> getLanguageSwitcher( const engine4heroes::TextBase & text )
    {
        ...
        return std::make_unique<engine4heroes::LanguageSwitcher>( language.value() );
    }
}
...
namespace engine4heroes
{
    ...
    size_t TextInput::getCursorPositionInAdjacentLine( const size_t currentPos, const int32_t maxWidth, const bool moveUp )
    {
        ...
        const engine4heroes::FontCharHandler charHandler( _fontType );
