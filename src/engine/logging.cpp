...
namespace Logging
{
    ...
    void InitLog()
    {
        ...
        logFile.open( "engine4heroes.log", std::ofstream::app );
#elif defined( _WIN32 )
        const std::scoped_lock<std::mutex> lock( logMutex );
        ...
        logFile.open( System::concatPath( configDir, "engine4heroes.log" ), std::ofstream::app );
#elif defined( MACOS_APP_BUNDLE )
        openlog( "engine4heroes", LOG_CONS | LOG_NDELAY, LOG_USER );
