...
public:
    ...
    void setGlobalMouseMotionEventHook( std::function<engine4heroes::Rect( const int32_t, const int32_t )> hook )
    {
        _globalMouseMotionEventHook = std::move( hook );
    }

    void setGlobalKeyDownEventHook( std::function<void( const engine4heroes::Key, const int32_t )> hook )
    {
    ...
    const engine4heroes::Point & getMouseCursorPos() const
    {
        return _mouseCursorPos;
    }

    const engine4heroes::Point & getMouseLeftButtonPressedPos() const
    {
    ...
    bool MouseClickLeft( const engine4heroes::Rect & area );
    bool MouseClickRight( const engine4heroes::Rect & area );
    ...
    bool MouseLongPressLeft( const engine4heroes::Rect & rt );
    ...
    bool isMouseLeftButtonPressedInArea( const engine4heroes::Rect & area ) const
    {
        return isMouseLeftButtonPressed() && ( area & _mousePressLeftPos );
    }

    bool isMouseLeftButtonPressedAndHeldInArea( const engine4heroes::Rect & area ) const
    {
    ...
    bool isMouseRightButtonPressedInArea( const engine4heroes::Rect & area ) const
    {
    ...
    bool isMouseLeftButtonReleasedInArea( const engine4heroes::Rect & area ) const
    {
        return isMouseLeftButtonReleased() && ( area & _mouseReleaseLeftPos );
    }

    bool isMouseWheelUpInArea( const engine4heroes::Rect & area ) const
    {
        return isMouseWheelUp() && ( area & _mouseCursorPos );
    }

    bool isMouseWheelDownInArea( const engine4heroes::Rect & area ) const
    {
        return isMouseWheelDown() && ( area & _mouseCursorPos );
    }

    bool isMouseCursorPosInArea( const engine4heroes::Rect & area ) const
    {
    ...
    bool isKeyPressed( const engine4heroes::Key key ) const
    {
    ...
    engine4heroes::Key getPressedKeyValue() const
    {
...
private:
    ...
    class LongPressDelay final : public engine4heroes::TimeDelay
    {
    public:
    ...
    engine4heroes::Key _currentKeyboardValue{ engine4heroes::Key::NONE };
    MouseButtonType _currentMouseButton{ MouseButtonType::MOUSE_BUTTON_UNKNOWN };

    engine4heroes::Point _mousePressLeftPos;
    engine4heroes::Point _mousePressMiddlePos;
    engine4heroes::Point _mousePressRightPos;

    engine4heroes::Point _mouseReleaseLeftPos;
    engine4heroes::Point _mouseReleaseMiddlePos;
    engine4heroes::Point _mouseReleaseRightPos;

    engine4heroes::Point _mouseCursorPos;

    engine4heroes::Point _mouseWheelMovementOffset;

    LongPressDelay _mouseButtonLongPressDelay;

    std::function<engine4heroes::Rect( const int32_t, const int32_t )> _globalMouseMotionEventHook;
    std::function<void( const engine4heroes::Key, const int32_t )> _globalKeyDownEventHook;

    engine4heroes::Rect _mouseCursorRenderArea;
    ...
    engine4heroes::PointBase2D<double> _emulatedPointerPos;
    ...
    engine4heroes::Time _controllerTimer;
    ...
    void onKeyboardEvent( const engine4heroes::Key key, const int32_t keyModifier, const KeyboardEventState keyState );
    void onMouseWheelEvent( engine4heroes::Point position );
    ...
    void onTouchFingerEvent( const TouchFingerEventType eventType, const int64_t touchId, const int64_t fingerId, engine4heroes::PointBase2D<float> position );
